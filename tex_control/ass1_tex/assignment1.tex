...
\pagenumbering{arabic}
	...
	\end{equation}
	\noindent The generic coefficients keep a direct link to the physical parameters (with units): $a=K$ [V\,s/rad], $b=J_m L_a$ [kg\,m$^2$H], $c=J_m R_a + b L_a$ [kg\,m$^2\Omega$ + N\,m\,s\,H/rad], and $d=bR_a+K^2$ [N\,m\,s\,\Omega/rad + V$^2$\,s$^2$/rad$^2$]. Under ZOH with sampling time $T_s$, the discrete second-order core model in~\eqref{eq:discrete_base_model} has coefficients $a_0 = e^{-\omega_n T_s}$, $a_1 = -2 e^{-\zeta \omega_n T_s}\cos(\omega_d T_s)$, and $b_0,b_1$ given by the standard ZOH bilinear mapping $B_d = \int_0^{T_s} e^{A\tau}B\,d\tau$, so $a_1,a_2,b_1,b_2$ in~\eqref{eq:discrete_full_model} are uniquely determined by $(J_m,b,K,L_a,R_a,T_s)$ and the one-sample microOS delay $z^{-1}$.
