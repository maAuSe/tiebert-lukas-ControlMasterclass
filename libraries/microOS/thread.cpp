...
Thread::Thread(priority_t priority, uint32_t period, int (*Fcn)()) :
  _active(false), _ID(0), _priority(priority), _period(period),
  _scheduling_time(0), _duration(0), _latency(0), _total_duration(0),
  _total_latency(0), _number_of_executions(0), _pFcn(Fcn)
{
  //do nothing
}
