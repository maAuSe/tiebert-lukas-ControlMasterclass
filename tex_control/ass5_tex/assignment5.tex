...
\end{align}
On our cart the measured geometry is $a = \SI{0.083}{m}$ (half the wheelbase), $\alpha = \SI{0.075}{m}$, $\beta = \SI{0.065}{m}$, and $\gamma = \SI{0.078}{m}$, with wheel radius $r = \SI{0.033}{m}$ (same value as in Assignment 3).
...
\begin{align*}
  Q &= \text{diag}\big(6.4\times 10^{-7},\; 6.4\times 10^{-7},\; 2.56\times 10^{-6}\big)\;[\text{m}^2,\text{m}^2,\text{rad}^2],\\
  R &= \text{diag}\big(6.4\times 10^{-7},\; 6.4\times 10^{-7}\big)\;[\text{m}^2],
\end{align*}
with $\hat{P}_{0|0} = \text{diag}\big((0.20)^2,\; (0.20)^2,\; (10^\circ)^2\big)$ and $\hat{\xi}_{0|0} = [-0.30\;-0.20\;0]^T$. These numbers exactly match the firmware constants ($kQScale=1$, $kQMultiplier=3.2\times 10^{-3}$, $kRScale=5$, $kRMultiplier=3.2\times 10^{-4}$ in \texttt{extended\_kalman\_filter.cpp}) so that the MATLAB replays stay in sync with what runs on the cart. The initial variance reflects the fact that the cart is placed around $(-0.30,-0.20)$~m with an approximate forward orientation but not precisely measured. The ratio between $Q$ and $R$ trades off dead-reckoning autonomy versus noise rejection; increasing $Q$ forces the EKF to rely more heavily on forthcoming measurements.
...
Offline re-runs of the same logs with wide Q/R sweeps revealed a reproducible jump in $\hat{\theta}$ as soon as button 2 starts the trajectory: the heading estimate steps to a positive bias and holds that level until the cart begins the programmed turn. This is driven by a high Kalman gain on the first range measurements after start-up (large $P_0$ combined with the small $R$ above) while the two wall distances are not perfectly consistent with the assumed offsets $\alpha,\beta,\gamma$ and wall placement. The filter resolves the geometric mismatch by rotating the pose. Inflating $R$ for the first second of motion or re-checking the sensor offsets/wall parameters removes the step; reducing $Q$ only slows the drift but does not eliminate the bias because the innovation is deterministic. A simple mitigation is to gate the measurement update or temporarily scale $R$ up during the acceleration phase when button 2 is pressed.
