...
#define PEN_TO_RAD      2.0 * M_PI / 1023.0 / 4.0  // transform encoder counts to radians (4096 enc per 2*pi)
...
class MECOtron : public HALBase {
  protected:
    ...
    Sensor1D* _pendulum  = new EncoderSensor(PEN_ENC_A, PEN_ENC_B);

    float _hbridge_buffer[2] = { 0.0, 0.0 };

    // Buttons
    bool _button_states[4] = {false, false, false, false};
  ...
  public:
    ...
    inline void writeValue(const uint8_t nr, const float val) { System.setGPoutFloat(nr, val); }
    inline float readValue(const uint8_t nr) { return System.getGPinFloat(nr); }
