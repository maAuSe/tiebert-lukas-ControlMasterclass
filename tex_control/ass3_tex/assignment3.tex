...
\begin{document}
  ...
  The goal of this assignment is to control the position of the cart along a straight line. The state is the cart position $x(t)$ [$m$] along the $x$-axis, with the wall at the origin $x=0m$. 
 ...
 \noindent The system parameters are:
  ...
  \noindent The input $u(t) = \omega(t)$ [$rad/s$] is the common wheel angular velocity setpoint, applied equally to both motors via the inner velocity controllers designed in Assignment~2. 
  ...
  \begin{equation}
    \boxed{z_\text{cl}(K) = 1 - T_s r K = 1 - 0.1\text{s} \cdot 0.033\text{m} \cdot K.}
  \end{equation}
  ...
  \noindent\textbf{Yes, the system can become unstable} if $K > 6060 rad/(s\cdotm)$, causing the pole to exit the unit circle through $z = -1$.
  ...
  1. Correct initial estimate
  If $\hat{x}[0] ≈ x[0]$, the estimator error is nearly zero, so:
  \begin{itemize}
  ...
  2. Incorrect initial estimate
  Because the chosen estimator pole is very slow:
  \begin{equation}
    e[k+1] = z_{est} e[k], \quad \text{with} z_{est} ≈ z_{cl}^{1/10} ≈ XXX,
  \end{equation}
  the error decays only gradually.
  During this long transient, the controller acts on:
  \begin{equation}
    x_{ref}[k] - \hat{x}[k],
  \end{equation}
  ...
  This demonstrates that a too-slow estimator harms the transient performance,
  especially when initial estimation erros are present.

  3. If you designing a state estimator yourself (using pole placement):
  ...
  For example, if the controller pole is $z_{cl} = 0.2$, a good choice for the estimator pole is around $z_{est} ≈ 0.1$ (or even smaller),
  which corresponds to an $L$ that gives very quick convergence of $\hat{x}[k]$ without excessive noise amplification.
  ...
  Its eigenvalues are:
  \begin{equation}
    \lambda_{1} = 1 - T_s r K = z_{\text{cl}} \text{(controller pole)}, \quad \lambda_{2} = 1 + L = z_{\text{est}} \text{(estimator pole)}.
  \end{equation}
  So the full closed-loop system has two poles: one equal to the controller pole and one equal to the estimator pole.
  ...
  assuming the internal states are already consistent (i.e. $\hat{x} ≈ x$). 
  ...
  The estimator performance depends on the estimator pole $z_{cl}$ \textit{and} the initial estimation error $e[0] = x[0] - \hat{x}[0]$,
  ...
  choosing $z_{est}$ much slower than $z_{cl}$:
  \begin{itemize}
    \item severely reduces closed-loop performance,
    \item introduces long transients when $\hat{x}[0]$ is wrong,
    \item demonstrates why estimators are typically placed faster than controllers, not slower.
  \end{itemize}
