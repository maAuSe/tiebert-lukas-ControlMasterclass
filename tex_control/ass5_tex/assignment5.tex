...
\\ \\
\textit{(Apply the reference trajectory to the closed-loop system and make a plot of the tracking errors of
x, y and $\theta$ for varying $Q$ and $R$ (again max. 4 combinations). Do not use any feedforward signals now (as opposed to the open-loop experiments of 2(c) where you exclusively applied feedforward
signals). \\ \\
...
\textit{(Explain the eﬀect of the diﬀerent choices of $Q$ and $R$ on the observed controller behavior based on these plots. How does it influence the convergence speed of the controller? Are there limitations
...
\\ \\
\textit{(Report the chosen numerical value of Q and R and motivate your choice. Do not forget their
	units!)}
