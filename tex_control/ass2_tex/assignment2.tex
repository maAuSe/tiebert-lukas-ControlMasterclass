...
\section{Velocity controller design using the frequency response method}

\subsection{Controller type choice (1a)} % Section 1(a)

To achieve zero steady-state error on a constant velocity reference, which requires a closed-loop system with type 1 behaviour, the controller must provide infinite DC gain.
...
\\ 
Desired closed-loop properties include: zero steady-state error on a constant velocity reference, adequate robustness (sufficient phase margin), sufficient closed-loop bandwidth $\omega_{BW}$ for accurate velocity tracking, and low sensitivity to external disturbances (e.g. cart on incline, friction).
\\
...
The continuous-time transfer function for the PI compensator is \cite{CT-slides-C8}:
\begin{equation}
...
The controller parameters are obtained using the frequency-response method, 
based on the identified motor models from Assignment 1, both for motor A and motor B.
The \textbf{unfiltered simplified discrete-time models} are:


\begin{equation}
	H_A(z) = \frac{0.6309}{z^2 - 0.6819 z}, 
\end{equation}
\begin{equation}
	H_B(z) = \frac{0.6488}{z^2 - 0.6806 z}.
\end{equation}
Transforming these discrete-time models to continuous-time (with \textit{MATLAB}'s \textit{d2c} command) yields:
\begin{equation}
    \boxed{G_{s,A}(s) = \frac{0.3677s^2 - 147.1s + 1.471e04}{s^2 + 237.1s + 7413},}
\end{equation}
\begin{equation}
    \boxed{G_{s,B}(s) = \frac{0.3763s^2 - 150.5s + 1.505e04}{s^2 + 237s + 7408}.}
\end{equation}


\paragraph{Choice of cross-over frequency $\omega_c$:} The cross-over frequency $\omega_c$ determines the closed-loop bandwidth $\omega_{BW}$. The design value must balance: faster response vs. actuator limitations, tracking performance vs. noise sensitivity, and stability margin vs. speed.
\\ 
A phase margin target of 55° was imposed to ensure robust behaviour.
The uncompensated phase of the motor model is evaluated to find the frequency satisfying:
\begin{equation}
    \phi_G(\omega_c) = -180^\circ + PM + \phi_{PI}(\omega_c),
\end{equation}
where $\phi_G(\omega_c)$ is the phase of the uncompensated open-loop system at the target cross-over frequency $\omega_c$, $PM$ is the desired phase margin (which is selected to be 55°), and $\phi_{PI}(\omega_c)$ is the anticipated phase lag of the PI compensator at $\omega_c$ (which is selected to be 15°).
\\ 
...
\begin{equation}
    \boxed{\omega_{c,A} = \omega_{c,B} = 61.7\; \text{rad/s}.}
...
The integration time is selected such that the contribution of the PI compensator to the phase at $\omega_c$ equals the anticipated one (this being $\phi_{PI}(\omega_c)$ = 15°).
...
\begin{equation}
    \boxed{T_{i,A} = T_{i,B} = 0.0605\; \text{s}.}
	%T_{i,\text{low}} = 1.19~\text{s}
\end{equation}


\paragraph{Proportional gain $K$:} The proportional gain is calculated such that the gain of the compensated system at the target cross-over frequency $\omega_c$ equals one:
\begin{equation}
...
\begin{itemize}
    ...
    \item increasing $K$ increases bandwidth but reduces phase margin, raising the risk of oscillatory behaviour and actuator saturation
...
\begin{itemize}
    \item zero located at $\omega_z = \frac{1}{T_i} = 16.5\:rad/s$, far below the target cross-over frequency $\omega_c$ ($\omega_z << \omega_c$).
\end{itemize}
...
\end{itemize}
Figures \ref{fig:Verification-Open-loop-A} and \ref{fig:Verification-Open-loop-B} show that, both for motor A (left) and motor B (right), the desired phase margin ($PM$) of 55° is approximately obtained, and the gain of $L(j\omega)$ is almost equal to $0\;dB$ at the target cross-over frequency $\omega_c$. The gain margin ($GM$) is also indicated on these plots, which is sufficiently large. In the Bode plot, the $-180^\circ$ phase crossing occurs at a frequency well above the cross-over frequency $\omega_c$. At that higher frequency, the open-loop magnitude is well below $0\;dB$, meaning a substantial increase in gain would be required to reach instability.
\\
Figures \ref{fig:Verification-PI-A} and \ref{fig:Verification-PI-B} show the PI controller indeed has a zero at $\omega_z$. The PI zero is identified at the frequency where the phase equals $-45^\circ°$ and the magnitude is $3\;dB$ above the high-frequency gain. Both for motor A (left) and motor B (right), the phase at $\omega_z$ is approximately $-45^\circ°$, and the magnitude at $\omega_z$ is approximately $3\;dB$ above the high-frequency plateau $20log_{10}K \approx -1\;dB$.
\\ 
Figures \ref{fig:Verification-Closed-loop-A} and \ref{fig:Verification-Closed-loop-B} show the desired closed-loop characteristics, both for motor A (left) and motor B (right). The resonant peak near the closed-loop bandwidth $\omega_{BW}$ is clearly visible, and the gain decreases beyond $\omega_{BW}$, indicating attenuation of high-frequency signals.
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{Figures_part_1b/Verification-Open-loop-B.png}
...
\noindent In Figures \ref{fig:Verification-Time-A} and \ref{fig:Verification-Time-B}, the step response converges to the reference value in steady-state, demonstrating the effectiveness of the integral action in eliminating the steady-state error, both for motor A (left) and motor B (right). The transient behaviour satisfies the design specifications, with rise time $t_r$ and overshoot $M_p$	remaining within the desired limits.
...
The closed-loop bandwidth $\omega_{BW}$ cannot exceed the frequency where the open-loop phase $\phi_L$ approaches $-180^\circ$. 
...
\begin{itemize}
	...
	\item voltage saturation (at $\approx$ 12 $V$) limits the achievable proportional gain
	\item encoder quantization noise leads to large high-frequency disturbances when the bandwidth is too high
	\item motor friction and unmodeled nonlinearities reduce agreement with the designed frequency response
\end{itemize}
These constraints limit the achievable closed-loop bandwidth $\omega_{BW}$ even if theoretically higher values are possible.
...
A constant velocity reference of $10\;rad/s$ is applied to both motors A and B. The measured velocity is compared to the simulated velocity (using the compensated closed-loop model $H(s)$).
...
Despite these differences, the measured response remains consistent with expected closed-loop behaviour.
...
Overall, the measured performance agrees with the designed characteristics: fast but well-damped convergence, full elimination of steady-state error, and control behaviour that remains within actuator limits.
...
The step reference (dotted line), measured closed-loop response (solid line) and simulated closed-loop response (dashed line) are shown in one plot.
The measured response shows slightly lower peaks and a slower rise than the nominal case (without disturbance), 
...
\paragraph{Tracking error with constant disturbance:} Figure \ref{fig:tracking_error_with_disturbance} shows the tracking error comparison with disturbance for both motor A (left) and motor B (right).
...
\paragraph{Control signal with constant disturbance:} Figure \ref{fig:control_signal_with_disturbance} shows the control signal comparison with disturbance for both motor A (left) and motor B (right).
...
\paragraph{Experiment:} 
With the redesigned PI controller, the cross-over frequency $\omega_c$ is reduced to about $0.5\;Hz$ or
\begin{equation}
	\boxed{\omega_c^{low-BW} \approx 3.14\;rad/s,}
\end{equation}
...
Using the identified values of the proportional gain $K$ 
\begin{equation}
	\boxed{K_A^{low-BW} = 0.4886, \quad K_B^{low-BW} = 0.4771,}
\end{equation}
and integration time 
\begin{equation}
	\boxed{T_{i,A}^{low-BW} = T_{i,B}^{low-BW} = 1.1879\;s,}
\end{equation}
the resulting low-bandwidth PI controllers for motor A and motor B are given by:
\begin{equation}
...
\begin{figure}[htbp]
    ...
    \end{subfigure}
    \caption{Step response comparison for wheel A (left) and wheel B (right): nominal controller vs. low-bandwidth controller, both under constant force disturbance.}
...
\begin{figure}[htbp]
    ...
    \end{subfigure}
    \caption{Tracking error comparison for wheel A (left) and wheel B (right): nominal controller vs. low-bandwidth controller, both under constant force disturbance.}
    \label{fig:tracking_error_high_vs_low_BW}
\end{figure}


\paragraph{Control signal with low-bandwidth (and constant disturbance):} Figure \ref{fig:control_signal_high_vs_low_BW} shows the control signal comparison for both motor A (left) and motor B (right).
...
The measured control signal of the low-bandwidth controller has clearly smaller peaks and a smoother shape. Although both controllers must compensate the same constant disturbance torque, the low-bandwidth controller converges to a slightly lower steady-state voltage in the measurements, likely due to reduced sensitivity to noise and unmodeled dynamics.
...
\paragraph{Contribution of the different (PI) controller parts:} For the nominal design, the proportional term (P) drives rapid acceleration and overshoot, while the integrator (I) eliminates any residual steady-state error under disturbance. In the low-bandwidth design, reduced gain $K$ and lower $\omega_c$ cause the proportional action to react more slowly, with the integral term building more gradually. This results in smoother, better-damped responses and lower peak control effort, though convergence to the reference is slower.

\newpage
\bibliographystyle{plain}
\bibliography{references-assign-2}
