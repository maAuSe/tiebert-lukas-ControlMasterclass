...
\noindent This model assumes ideal velocity control (the velocity control loop is closed internally) and no wheel slip.
...
For the configuration shown in Figure~\ref{fig:figure-assignment-5}, wall~1 (horizontal, top) is defined by $y = 0$, corresponding to $(p_1, q_1, r_1) = (0, 1, 0)$. Wall~2 (vertical, left) is defined by $x = 0$, corresponding to $(p_2, q_2, r_2) = (1, 0, 0)$. Substituting into \eqref{eq:signed_distance}:
\begin{align}
...
\noindent In contrast, a standard linear Kalman filter (KF) would use a fixed linearisation point, typically chosen as the initial state or a nominal operating point. The EKF adapts its linear approximation as the state evolves, whereas the linear KF maintains constant system matrices. For trajectories involving significant heading changes (such as the $90^\circ$ turn in this assignment), the fixed-linearisation approach introduces massive model mismatch, intensely degrading filter performance.
...
\begin{equation}
	\boxed{Q = \operatorname{diag}(8 \times 10^{-9}, \, 9 \times 10^{-8}, \, 9 \times 10^{-7}) \; [\text{m}^2, \text{m}^2, \text{rad}^2],} 
	\label{eq:tuned_Q} 
\end{equation}
\begin{equation}
	\boxed{R = \operatorname{diag}(0.0198, \, 0.09) \; [\text{m}^2].}
	 \label{eq:tuned_R}
\end{equation}
...
\end{figure}
\noindent The 95\% bands reflect the tuned $R$ values. 
...
\begin{equation}
	\boxed{Q_{\text{lqr}} = \operatorname{diag}(64, 64, 16) \; [\text{m}^{-2}, \text{m}^{-2}, \text{rad}^{-2}],} 
	\label{eq:chosen_Qlqr} 
\end{equation}
\begin{equation}
	\boxed{R_{\text{lqr}} = \operatorname{diag}(0.05, 0.05) \; [\text{s}^2/\text{m}^2, \text{s}^2/\text{rad}^2].}
	\label{eq:chosen_Rlqr}
\end{equation}
