...
\paragraph{Contribution of the different (PI) controller parts:} For the nominal design, the proportional term (P) drives rapid acceleration and overshoot, while the integrator (I) eliminates residual steady-state error under disturbance. In the low-bandwidth design, reduced gain $K$ and lower $\omega_c$ cause the proportional action to react more slowly, with the integral term building more gradually. This results in smoother, better-damped responses and lower peak control effort, though convergence to the reference is slower.
