...
\end{equation}
where $\mathbf{w}_k \sim \mathcal{N}(\mathbf{0}, Q)$ represents additive process noise. \\ \\
The measurement equation, being algebraic rather than differential, requires no discretisation:
\begin{equation}
...
\end{itemize}
No feedforward signals were applied; the closed-loop system relied exclusively on state feedback. The resulting tracking errors in the world frame and body frame are shown in Figures~\ref{fig:lqr_errors_world} and~\ref{fig:lqr_errors_body}, respectively. The corresponding control signals are shown in Figure~\ref{fig:lqr_control}.

\begin{figure}[H]
	\centering
	\includegraphics[width=0.95\linewidth]{images/lqr_tracking_errors_world.pdf}
	\caption{World-frame tracking errors $e_x$, $e_y$, $e_\theta$ for four LQR tuning configurations. The trajectory comprises an initial transient, a straight segment ($t < 5$~s), a $90^\circ$ turn ($5 < t < 9$~s), and a final straight segment ($t > 9$~s).}
	\label{fig:lqr_errors_world}
\end{figure}

\begin{figure}[H]
	\centering
	\includegraphics[width=0.95\linewidth]{images/lqr_tracking_errors_body.pdf}
	\caption{Body-frame tracking errors $e'_x$, $e'_y$, $e'_\theta$ for the same four LQR configurations. These errors are the inputs to the LQR controller after the rotation~\eqref{eq:rotation_matrix}.}
	\label{fig:lqr_errors_body}
\end{figure}

\begin{figure}[H]
	\centering
	\includegraphics[width=0.95\linewidth]{images/lqr_control_signals.pdf}
	\caption{Control signals $v$ and $\omega$ for the four LQR configurations. The signals reflect the feedback corrections applied to track the reference trajectory.}
