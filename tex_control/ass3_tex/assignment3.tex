...
\begin{document}
  ...
  \begin{itemize}
    ...
    \item Large $K$ (near upper stability limit, $K$ ≈ $\frac{2}{T_s r}$): pole $z_\text{cl}$ approaches -1, oscillatory (sign-alternating) behaviour in discrete-time with long settling and potential overshoot.
  \end{itemize}
  ...
  \paragraph{Limitations on $K$:}
  There are both theoretical and practical limits.
  ...
  Practical limits:
  \begin{itemize}
    \item The low-level motor voltage is bounded: too large $K$ causes saturation, making the actual behaviour deviate from the linear model.
  ...
  \subsection{Estimator and controller (2c)} % Section 2(c)
