...
\begin{equation}
	\hat{P}_{0|0} = \operatorname{diag}(1 \times 10^{-7}, \, 1 \times 10^{-7}, \, 1 \times 10^{-7}) \; [\text{m}^2, \text{m}^2, \text{rad}^2].
	\label{eq:initial_covariance}
\end{equation}

These values reflect the practical placement accuracy achievable when positioning the cart manually: the translational entries correspond to approximately $\pm 0.3$~mm ($1\sigma$) placement accuracy, while the yaw entry corresponds to approximately $0.02^\circ$ ($1\sigma$) alignment uncertainty. These conservative initial variances ensure the filter rapidly converges to the true state once measurements become available.
...
\subsection{Measurement Predictions and Confidence Intervals}
...
The 95\% bands reflect the tuned $R$ values. The bands widen during the turn when measurements are disabled and the filter relies solely on prediction. After the turn, only $z_1$ contributes to the correction, leaving residual uncertainty in states not directly observed by the front sensor.
