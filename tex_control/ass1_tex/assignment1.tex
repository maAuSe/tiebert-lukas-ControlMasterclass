...
\pagenumbering{arabic}
	\section{Model structure selection for the cart}
	\subsection{Discrete-time model structure selection (1a)}
...
\end{equation}
	...
	\subsubsection{Model structure motivation}
	...
	\subsubsection{Link between continuous and discrete coefficients}
	
	The generic coefficients $(a,b,c,d)$ in~\eqref{eq:CT_transfer_function_DC} relate to the physical parameters via
	\begin{equation}
		a = K, \quad b = J_m L_a, \quad c = J_m R_a + b L_a, \quad d = b R_a + K^2.
		\label{eq:generic_to_physical}
	\end{equation}
	Both this mapping and the ZOH discretization are bijective for fixed $T_s$; hence the identified discrete-time coefficients can be inverted to recover the underlying motor constants whenever physical interpretation is required.
	
	
	\subsection{Model parameters estimation (1b)}
	...
	\subsubsection{Recursion expression and error criterion}
	
	The difference equation for the third-order model is \cite{CT-slides-C4}:
	\begin{equation}
...
\begin{equation}
	\boxed{\hat{H}^{\text{simp}}_A(z) = \frac{0.6309}{z^2 - 0.6819 z}, \qquad \hat{H}^{\text{simp}}_B(z) = \frac{0.6488}{z^2 - 0.6806 z}.}
\end{equation}
...
\begin{equation}
	\boxed{\hat{H}_A(z) = z^{-1} \hat{H}_{0,A}(z) = \frac{0.007672 z + 0.6589}{z^{3} - 0.5671 z^{2} - 0.09698 z},}
\end{equation}
\begin{equation}
	\boxed{\hat{H}_B(z) = z^{-1} \hat{H}_{0,B}(z) = \frac{0.008073 z + 0.6741}{z^{3} - 0.5762 z^{2} - 0.08815 z}.}
\end{equation}
...
\noindent The two motors therefore behave almost identically: the RMS prediction errors differ by less than 0.004~rad/s (below 1\%) for both model structures, and the identified parameters vary by less than 2\% across motors. The model-measurement errors in Figure~\ref{fig:model_error_pair} overlap as well, showing that the remaining discrepancies stem from shared effects such as sensor resolution/quantization limitations. Because the validation metrics and visual inspections point to equivalent dynamics, subsequent sections display detailed plots for motor~A only; whenever performance numbers are quoted, both motors are still reported to document their similarity.
...
\begin{equation}
	\boxed{\hat{H}^{\text{simp,filt,20}}_A(z) = \frac{0.5940}{z^2 - 0.7007 z}, \qquad \hat{H}^{\text{simp,filt,35}}_A(z) = \frac{0.6204}{z^2 - 0.6873 z}.}
\end{equation}
...
\begin{equation}
	\boxed{\hat{H}_{A}^{\text{filt,20}}(z) = z^{-1} \hat{H}_{0,A}^{\text{filt,20}}(z) = \frac{0.3416 z - 0.07297}{z^{3} - 1.338 z^{2} + 0.4738 z},}
\end{equation}
\begin{equation}
	\boxed{\hat{H}_{A}^{\text{filt,35}}(z) = z^{-1} \hat{H}_{0,A}^{\text{filt,35}}(z) = \frac{-0.01271 z + 0.5937}{z^{3} - 0.7993 z^{2} + 0.09234 z}.}
\end{equation}
...
\bibliography{references-assign-1}
...
\appendix
	...
	\noindent This model has two parameters ($a_1, b_1$) compared to four in the full model.\newline
	
	\noindent The recursion expression for the simplified model is given in the main text by \eqref{eq: difference eq simple}, with the associated regression vector and least-squares criterion \eqref{eq: LS criterion}. For completeness, we retain the matrix formulation below.
