...
\textbf{Implementation Constraints:} Digital implementation imposes hard limit via sampling rate. The chosen bandwidth $\omega_{BW} \approx 4.8~\text{Hz}$ balances responsiveness with robustness and practical constraints.

\textbf{Actuator Clamping:} The MECOtron libraries (\texttt{libraries/control-theory/mecotron.h}) drive the Pololu TB6612FNG stage from a 12~V battery rail. To stay within the safe operating window and allow for sensing offsets, the firmware clamps the commanded voltage to $\pm 11$~V, which matches the plateaus visible in the measured control signals.

\section{Experimental Validation}

All tests were executed on the MECOtron v3 cart controlled at 100~Hz. The firmware streamed reference, speed, error, control, and mode flags through QRC, and the MATLAB script in \texttt{matlab\_assign2/assignment2\_solution.m} post-processed the resulting logs. The following CSV files are used throughout the section: \texttt{cart\_flat\_step.csv} (flat ground), \texttt{cart\_incline\_nominal.csv} (incline with nominal controller), and \texttt{cart\_incline\_lowband.csv} (incline with the low-bandwidth controller).

\subsection{Flat-Ground Step (Specification 2a)}

\textbf{Experiment:} Both wheels track a $0 \rightarrow 10$~rad/s velocity command using the nominal (30~rad/s) PI controller. The rise and settling times extracted from the log are $0.32$~s and $0.65$~s, respectively, which agree with the discrete-time model predictions (0.29~s and 0.64~s). Steady-state error remains below $0.05$~rad/s for the full duration of the experiment, confirming the correct integral action.

\textbf{Step Response:} Simulated and measured velocities overlap within $\pm 0.2$~rad/s once the loop leaves the saturation regime. The transient is dominated by the intentionally aggressive crossover and shows no overshoot, which is consistent with the 55° phase margin requirement. Differences between Motor~A and Motor~B are below the encoder noise floor.

\textbf{Tracking Error:} The logged error channels decay exponentially after an initial spike caused by the sample-and-hold delay. Within 0.4~s the absolute error is bounded by 0.05~rad/s for both wheels, matching the performance predicted by the model-based sensitivity function.

\textbf{Control Signal and Clamping:} The control effort saturates at $\pm 11$~V for roughly 150~ms, matching the limit imposed in firmware to keep the TB6612FNG bridge within the 12~V rail defined in \texttt{libraries/control-theory/mecotron.h}. After the transient, each wheel settles around 3--4~V to compensate friction, and the measured voltages are a close match to the MATLAB control-signal simulation.
