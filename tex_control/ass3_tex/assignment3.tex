...
\noindent Table~\ref{tab:K_poles} and Figure~\ref{fig:step_K_sweep} confirm that larger $K$ moves $z_\text{cl}$ toward zero, yielding faster settling but higher control effort. For $K > K_\text{max}$, the pole exits the unit circle and the system becomes unstable.

\paragraph{Choice of $K$:}
Beyond the theoretical limit $K_\text{max} \approx 6060$ rad/(s$\cdot$m), practical constraints -actuator saturation ($\approx$11--12~V), sensor noise amplification, and IR range (5--30~cm)- are more restrictive. The selected value
\begin{equation}
    \boxed{K_{nom} = 40\ \text{rad/(s$\cdot$m)}}
\end{equation}
yields $z_\text{cl} = 0.9868$, providing $t_s \approx 3$~s with control effort within actuator limits.
...
Faster estimators (larger $|L|$, pole closer to 0) converge quickly but amplify measurement noise; slower estimators (pole closer to 1) reject noise but respond sluggishly to initialization errors. The estimator pole is typically placed 2--6 times faster than the controller pole. For Section~2.3, the specification requires setting it 10 times \emph{slower}.
...
Configuration:
\begin{itemize}
    \item estimator active, controller off
...
The experimental results confirm the theoretical predictions from Section~1.4: larger $|L|$ (pole closer to 0) yields faster convergence but amplifies sensor noise, while smaller $|L|$ (pole closer to 1) produces smoother but slower estimates. The chosen $L_\text{nom} = -0.18$ ($z_\text{est} = 0.82$) balances settling time and noise rejection.
...
\paragraph{Interpretation:}
The experimental step responses confirm the theoretical predictions from Section~1.3: higher $K$ yields faster settling but increases peak voltage. For $K_\text{fast} = 80$ rad/(s$\cdot$m), the voltage briefly saturates, causing mild overshoot. The selected $K_\text{nom} = 40$ rad/(s$\cdot$m) avoids saturation while achieving $t_s \approx 3$~s.
...
With $K_{nom} = 40$ rad/(s$\cdot$m), $z_\text{cl} = 0.9868$ (cf.\ Section~1.3).
...
\begin{figure}[H]\centering
    \subfloat[Good initial estimate]{%
      \includegraphics[width=0.46\textwidth]{images/est_ctrl_good.pdf}%
      \label{fig:est_ctrl_good}%
    }\hfill
    \subfloat[Wrong initial estimate]{%
      \includegraphics[width=0.46\textwidth]{images/est_ctrl_bad.pdf}%
      \label{fig:est_ctrl_bad}%
    }
    \caption{Combined estimator and controller. Left: good initial estimate; right: wrong initial estimate showing slow estimator convergence.}
\end{figure}
...
\paragraph{Separation principle and full closed-loop poles:}
The combined system has eigenvalues $\lambda_1 = z_\text{cl}$ and $\lambda_2 = z_\text{est}$, confirming the separation principle: controller and estimator poles appear independently in the closed-loop system. However, although the poles are decoupled, the controller acts on $\hat{x}[k]$ rather than $x[k]$. A slow estimator with wrong initialization causes $\hat{x}[k]$ to lag behind $x[k]$, degrading transient performance despite unchanged pole locations. The closed-loop transfer function describes only asymptotic behavior; it does not capture the effect of initial estimation error $e[0]$.

\paragraph{Conclusion:}
Choosing $z_\text{est}$ much slower than $z_\text{cl}$ severely degrades transient performance and introduces long settling times when $\hat{x}[0]$ is wrong---demonstrating why \textbf{estimators are typically placed faster than controllers}, not slower.
