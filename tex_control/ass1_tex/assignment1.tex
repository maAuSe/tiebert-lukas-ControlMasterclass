...
\noindent Filtering both the input voltage ($v[k]$) and output angular velocity ($\omega_A[k]$) with the same linear-phase filter maintains the correct phase relationship and gain ratio at all frequencies below the cutoff.
	...
	An eighth-order Butterworth filter was used because of its flat passband characteristic, and we swept the cutoff from 20~Hz up to the Nyquist limit of 50~Hz. None of those settings produced a meaningful improvement: the apparent ``high-frequency'' ripple is tied to quantization rather than a removable noise band, so even aggressive cutoffs fail to clean it up.\newline

\noindent Worse, every cutoff shaves off useful transient content (and phase information) all the way up to Nyquist, so the filtered signals systematically lose the sharp voltage-to-velocity steps that carry the identification information. Because no level of cutoff usefully removes the high-frequency noise and we always trade away informative transients, filtering cannot help here.
...
\begin{figure}[H]
 ...
 \end{minipage}
 \caption{Measured (black) versus simulated responses when identification uses filtered data. Left: full third-order model overlays for unfiltered, 20~Hz, and 35~Hz cutoffs. Right: the same comparison for the simplified model. Filtering softens the transients but does not eliminate the high-frequency artefacts.}
