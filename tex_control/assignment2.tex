...
\begin{equation}
    \omega_{c,\text{nom}} = 30 \texttt{rad/s} for motor A
    \omega_{c,\text{nom}} = YYY \texttt{rad/s} for motor B
...
\begin{equation}
    T_{i,\text{nom}} = 0.124 \texttt{s} for motor A
    T_{i,\text{nom}} = YYY \texttt{s} for motor B
...
\subsection{Steady-state performance under constant force disturbance}
...
Figure XXX shows the block diagram of the closed-loop system with the constant force disturbance.
...
\section{Controller validation with cross-over frequency of 0.5 Hz (including constant disturbance)}
