...
\begin{document}
	...
	\begin{enumerate}
		...
		\item \textit{No plagiarism has been committed as described on \url{https://eng.kuleuven.be/studeren/masterproef-en-papers/plagiaat\#Definitie:\%20wat\%20is\%20plagiaat^\circ}.}
...
\subsection{Controller type choice} % Section 1(a)
...
\subsection{Design process and design parameters choices} % Section 1(b)
...
\begin{equation}
    \omega_{c,A} = \omega_{c,B} = 60~\text{rad/s}, \qquad \omega_{c,\text{low}} = 2\pi \cdot 0.5~\text{rad/s}
\end{equation}
...
\begin{equation}
    T_{i,A} = T_{i,B} \approx 0.062~\text{s}, \qquad T_{i,\text{low}} \approx 1.19~\text{s}
\end{equation}
...
The identified wheel models yield proportional gains obtained from the unity-gain condition $\lvert D(j\omega_c)G(j\omega_c)\rvert = 1$; the MATLAB script prints these numerical values (for nominal and low-bandwidth controllers) and the same numbers are exported to the Arduino coefficients.
...
Figures~\ref{fig:openloop_bode} and \ref{fig:closedloop_bode} show the compensated open- and closed-loop responses for both motors. The open-loop plots highlight the unity-gain crossover at $\omega_c = 60~\text{rad/s}$ with a phase margin of roughly $55^\circ$, and the closed-loop plots show that the resulting bandwidth is about the chosen crossover with a modest resonant peak.
...
\subsection{Theoretical and practical closed-loop bandwidth limitations} % Section 1(c)
...
\begin{itemize}
	...
	\item Voltage saturation ($\approx 12~V$) limits achievable proportional gain.
