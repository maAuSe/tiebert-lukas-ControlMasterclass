...
Figure~\ref{fig:pole_map_K} shows the closed-loop pole location for varying $K$. The pole moves along the real axis from $z = 1$ (at $K = 0$) toward $z = -1$ (at $K = K_\text{max}$ = 6060 rad/(s $\cdot$ m)).
...
\begin{itemize}
    \item small $K$ ($z_\text{cl} \approx 1$): slow exponential decay, long settling time, smooth but sluggish response
    \item moderate $K$ ($0 < z_\text{cl} < 1$): faster convergence, monotonic response without oscillation
    \item large $K$ ($z_\text{cl} < 0$): sign-alternating (oscillatory) discrete-time response; if $|z_\text{cl}| < 1$, still stable but with overshoot
    \item $K > K_\text{max}$ ($|z_\text{cl}| > 1$): unstable, diverging oscillations
\end{itemize}
...
\paragraph{Design guideline:} The estimator pole should typically be 2--6 times faster than the controller pole, ensuring the estimation error decays before significantly affecting control performance.
...
\paragraph{Pole placement and trade-offs:} The state estimator gain $L$ is selected such that the estimator is $3$--$5$ times faster than the controller for tasks in Section~2.1--2.2, then set it ten times slower than the controller for Section~2.3 per the specification. The practical limits are set by sensor noise and discretization: $|1+L|<1\Rightarrow -2<L<0$.
...
\noindent Figure~\ref{fig:estimator_L_sweep} overlays the measured distance $y=-x$ and the estimated distance $-\hat{x}$ for $L\in\{-0.05,-0.18,-0.35\}$. Larger $|L|$ yields faster convergence but more noise on $\hat{x}$ and $\nu$.
...
\end{itemize}
For moderate $|L|$ (e.g. the designed $L \approx -0.05$, giving $z_{est} \approx 0.95$):
\begin{itemize}
...
The estimator is disabled, feedback uses the raw distance $y[k] = -x[k]$. A 0.25 m step reference is applied for $K\in\{K_{slow}, K_{nominal}, K_{fast}\}$ = \{20, 40, 80\} rad/(s $\cdot m)$. 
\\ \\
Configuration:
\begin{itemize}
...
\noindent Figure~\ref{fig:controller_K_response} shows the step reference (dashed line) and position responses for different values of $K$. Figure~\ref{fig:controller_K_voltage} shows the corresponding motor voltages, for the same values of $K$.
...
\end{equation}
For small $K$ (e.g. $K_{slow}$ = 20 rad/(s$\cdot$m)):
\begin{itemize}
...
\end{itemize}
For moderate $K$ (e.g. the designed $K_{nominal}$ = 40 rad/(s$\cdot$m)):
  \begin{itemize}
...
\end{itemize}
For large $K$ close to the theoretical maximum $\frac{2}{T_s r}$ (e.g. $K_{fast}$ = 80 rad/(s$\cdot$m)):
\begin{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{If you designing a state estimator yourself (using pole placement):}
...
The estimator performance depends on the estimator pole $z_{cl}$ \textbf{and} the initial estimation error $e[0] = x[0] - \hat{x}[0]$,
...
\begin{itemize}
    ...
    \item demonstrates why estimators are typically placed \textbf{faster} than controllers, not slower
\end{itemize}
